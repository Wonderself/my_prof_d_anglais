...
# --- LOGGING CONFIG ---
# On force les logs sur la sortie standard pour que Render les capture
...
# Important pour HTTPS sur Render
app.config['PREFERRED_URL_SCHEME'] = 'https'
app.wsgi_app = ProxyFix(app.wsgi_app, x_proto=1, x_host=1)

CORS(app)

# --- CSP HEADERS (CORRECTED) ---
@app.after_request
...
# --- HEALTH CHECK (LA PRIORITÉ ABSOLUE) ---
@app.route('/health')
def health():
    # Zéro logique complexe ici. Juste répondre "Je suis là".
...
# --- DB HELPERS (CONNECTION ONLY WHEN NEEDED) ---
...
# --- LAZY DB INIT (On demande à la DB de s'initialiser SEULEMENT aux actions critiques) ---
def ensure_tables_exist():
    # Cette fonction sera appelée uniquement lors du login ou du premier chat
    conn = get_db_connection()
    if not conn: return
    try:
        cur = conn.cursor()
    ...
    except Exception as e: logger.error(f"TABLE INIT ERROR: {e}")

# --- ROUTES MÉTIER ---

@app.route('/auth/login')
def login():
    ensure_tables_exist() # On initialise la DB juste avant le login
...
# ... (Le reste des fonctions AI/TTS est identique, je te laisse les mettre ici) ...
...
@login_required
def start_chat():
    ensure_tables_exist() # Sûreté si le user n'a pas refresh
